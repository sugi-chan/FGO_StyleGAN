...
def training_loop(
    ...
    resume_run_id           = 'C:/Users/micha/Desktop/projects/stylegan/results/00000-sgan-faces-1gpu/2019-03-08-stylegan-animefaces-network-02051-021980.pkl',     # Run ID or network pkl to resume training from, None = start from scratch.
    ...
    while cur_nimg < total_kimg * 1000:  #changed to test what happened 5/3
        #if ctx.should_stop():
        #    break
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000:# or done:
            ...
            if cur_tick % network_snapshot_ticks == 0 or cur_tick == 1:
                pkl = os.path.join(submit_config.run_dir, 'network-snapshot-%06d.pkl' % (cur_nimg // 1000))
                misc.save_pkl((G, D, Gs), pkl)
                #metrics.run(pkl, run_dir=submit_config.run_dir, num_gpus=submit_config.num_gpus, tf_config=tf_config)
            print('saved and continuting')
            # Update summaries and RunContext.
            #metrics.update_autosummaries()
            tflib.autosummary.save_summaries(summary_log, cur_nimg)
            ctx.update('%.2f' % sched.lod, cur_epoch=cur_nimg // 1000, max_epoch=total_kimg)
            maintenance_time = ctx.get_last_update_interval() - tick_time
            print('got to end of maintenance...')
            print(sched.tick_kimg ,cur_tick,cur_nimg,total_kimg)
